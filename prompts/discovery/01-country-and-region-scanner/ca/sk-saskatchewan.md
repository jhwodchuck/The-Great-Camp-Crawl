# Copy-Paste Discovery Prompt: CA / SK / Saskatchewan
...
- Save the gathered report to `reports/discovery/ca-sk-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-sk-country-region-scan`
- `country`: `CA`
- `region`: `SK`
...
- `"overnight camp" "Saskatchewan"`
- `"residential camp" "Saskatchewan"`
- `"camp with accommodation" "Saskatchewan"`
- `"family camp" "Saskatchewan"`
...
- Save the JSON to `reports/discovery/ca-sk-country-region-scan.json` if you can write files.
