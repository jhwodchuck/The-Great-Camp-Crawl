# Copy-Paste Discovery Prompt: CA / PE / Prince Edward Island
...
- Save the gathered report to `reports/discovery/ca-pe-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-pe-country-region-scan`
- `country`: `CA`
- `region`: `PE`
...
- `"overnight camp" "Prince Edward Island"`
- `"residential camp" "Prince Edward Island"`
- `"camp with accommodation" "Prince Edward Island"`
- `"family camp" "Prince Edward Island"`
...
- Save the JSON to `reports/discovery/ca-pe-country-region-scan.json` if you can write files.
