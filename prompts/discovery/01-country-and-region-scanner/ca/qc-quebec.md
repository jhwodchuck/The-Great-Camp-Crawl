# Copy-Paste Discovery Prompt: CA / QC / Quebec
...
- Save the gathered report to `reports/discovery/ca-qc-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-qc-country-region-scan`
- `country`: `CA`
- `region`: `QC`
- `region_name`: `Quebec`
...
- `"camp de vacances" "Quebec"`
- `"camp residentiel" "Quebec"`
- `"camp avec hebergement" "Quebec"`
- `"camp familial" "Quebec"`
- `site:.ca "Quebec" programme residentiel secondaire`
...
- Save the JSON to `reports/discovery/ca-qc-country-region-scan.json` if you can write files.
