# Copy-Paste Discovery Prompt: CA / ON / Ontario
...
- Save the gathered report to `reports/discovery/ca-on-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-on-country-region-scan`
- `country`: `CA`
- `region`: `ON`
- `region_name`: `Ontario`
...
- `"overnight camp" "Ontario"`
- `"residential camp" "Ontario"`
- `"camp with accommodation" "Ontario"`
- `"family camp" "Ontario"`
...
- Save the JSON to `reports/discovery/ca-on-country-region-scan.json` if you can write files.
