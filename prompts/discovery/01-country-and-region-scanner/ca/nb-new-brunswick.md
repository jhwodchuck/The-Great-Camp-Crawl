# Copy-Paste Discovery Prompt: CA / NB / New Brunswick
...
- Save the gathered report to `reports/discovery/ca-nb-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-nb-country-region-scan`
- `country`: `CA`
- `region`: `NB`
...
- `"overnight camp" "New Brunswick"`
- `"camp de vacances" "New Brunswick"`
- `"camp with accommodation" "New Brunswick"`
- `"camp residentiel" "New Brunswick"`
...
- Save the JSON to `reports/discovery/ca-nb-country-region-scan.json` if you can write files.
