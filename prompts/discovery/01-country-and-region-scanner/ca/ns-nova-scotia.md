# Copy-Paste Discovery Prompt: CA / NS / Nova Scotia
...
- Save the gathered report to `reports/discovery/ca-ns-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-ns-country-region-scan`
- `country`: `CA`
- `region`: `NS`
...
- `"overnight camp" "Nova Scotia"`
- `"residential camp" "Nova Scotia"`
- `"camp with accommodation" "Nova Scotia"`
- `"family camp" "Nova Scotia"`
...
- Save the JSON to `reports/discovery/ca-ns-country-region-scan.json` if you can write files.
