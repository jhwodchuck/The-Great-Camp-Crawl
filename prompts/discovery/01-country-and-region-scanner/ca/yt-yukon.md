# Copy-Paste Discovery Prompt: CA / YT / Yukon
...
- Save the gathered report to `reports/discovery/ca-yt-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-yt-country-region-scan`
- `country`: `CA`
- `region`: `YT`
- `region_name`: `Yukon`
...
- `"overnight camp" "Yukon"`
- `"residential camp" "Yukon"`
- `"camp with accommodation" "Yukon"`
- `"family camp" "Yukon"`
...
- Save the JSON to `reports/discovery/ca-yt-country-region-scan.json` if you can write files.
