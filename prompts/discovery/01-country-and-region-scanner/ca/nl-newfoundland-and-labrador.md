# Copy-Paste Discovery Prompt: CA / NL / Newfoundland and Labrador
...
- Save the gathered report to `reports/discovery/ca-nl-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-nl-country-region-scan`
- `country`: `CA`
- `region`: `NL`
...
- `"overnight camp" "Newfoundland and Labrador"`
- `"residential camp" "Newfoundland and Labrador"`
- `"camp with accommodation" "Newfoundland and Labrador"`
- `"family camp" "Newfoundland and Labrador"`
...
- Save the JSON to `reports/discovery/ca-nl-country-region-scan.json` if you can write files.
