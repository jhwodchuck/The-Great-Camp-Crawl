# Copy-Paste Discovery Prompt: CA / NU / Nunavut
...
- Save the gathered report to `reports/discovery/ca-nu-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-nu-country-region-scan`
- `country`: `CA`
- `region`: `NU`
- `region_name`: `Nunavut`
...
- `"overnight camp" "Nunavut"`
- `"residential camp" "Nunavut"`
- `"camp with accommodation" "Nunavut"`
- `"family camp" "Nunavut"`
...
- Save the JSON to `reports/discovery/ca-nu-country-region-scan.json` if you can write files.
