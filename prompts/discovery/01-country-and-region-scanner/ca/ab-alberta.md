# Copy-Paste Discovery Prompt: CA / AB / Alberta
...
- Save the gathered report to `reports/discovery/ca-ab-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-ab-country-region-scan`
- `country`: `CA`
- `region`: `AB`
- `region_name`: `Alberta`
...
- `"overnight camp" "Alberta"`
- `"residential camp" "Alberta"`
- `"camp with accommodation" "Alberta"`
- `"family camp" "Alberta"`
...
- Save the JSON to `reports/discovery/ca-ab-country-region-scan.json` if you can write files.
