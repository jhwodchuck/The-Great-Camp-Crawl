# Copy-Paste Discovery Prompt: CA / MB / Manitoba
...
- Save the gathered report to `reports/discovery/ca-mb-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-mb-country-region-scan`
- `country`: `CA`
- `region`: `MB`
...
- `"overnight camp" "Manitoba"`
- `"residential camp" "Manitoba"`
- `"camp with accommodation" "Manitoba"`
- `"family camp" "Manitoba"`
...
- Save the JSON to `reports/discovery/ca-mb-country-region-scan.json` if you can write files.
