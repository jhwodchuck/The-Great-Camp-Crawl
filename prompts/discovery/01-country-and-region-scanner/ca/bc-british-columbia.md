# Copy-Paste Discovery Prompt: CA / BC / British Columbia
...
- Save the gathered report to `reports/discovery/ca-bc-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-bc-country-region-scan`
- `country`: `CA`
- `region`: `BC`
...
- `"overnight camp" "British Columbia"`
- `"residential camp" "British Columbia"`
- `"camp with accommodation" "British Columbia"`
- `"family camp" "British Columbia"`
...
- Save the JSON to `reports/discovery/ca-bc-country-region-scan.json` if you can write files.
