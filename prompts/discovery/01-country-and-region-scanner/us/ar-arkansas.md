# Copy-Paste Discovery Prompt: US / AR / Arkansas
...
- Save the gathered report to `reports/discovery/us-ar-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ar-country-region-scan`
- `country`: `US`
- `region`: `AR`
...
- `"overnight summer camp" "Arkansas"`
- `"residential summer camp" "Arkansas"`
- `"family camp" "Arkansas"`
- `"youth retreat" "Arkansas" overnight`
...
- Save the JSON to `reports/discovery/us-ar-country-region-scan.json` if you can write files.
