# Copy-Paste Discovery Prompt: US / KS / Kansas
...
- Save the gathered report to `reports/discovery/us-ks-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ks-country-region-scan`
- `country`: `US`
- `region`: `KS`
- `region_name`: `Kansas`
...
- `"overnight summer camp" "Kansas"`
- `"residential summer camp" "Kansas"`
- `"family camp" "Kansas"`
- `"youth retreat" "Kansas" overnight`
...
- Save the JSON to `reports/discovery/us-ks-country-region-scan.json` if you can write files.
