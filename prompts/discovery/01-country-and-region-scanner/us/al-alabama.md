# Copy-Paste Discovery Prompt: US / AL / Alabama
...
- Save the gathered report to `reports/discovery/us-al-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-al-country-region-scan`
- `country`: `US`
- `region`: `AL`
- `region_name`: `Alabama`
...
- `"overnight summer camp" "Alabama"`
- `"residential summer camp" "Alabama"`
- `"family camp" "Alabama"`
- `"youth retreat" "Alabama" overnight`
...
- Save the JSON to `reports/discovery/us-al-country-region-scan.json` if you can write files.
