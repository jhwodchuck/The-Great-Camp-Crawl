# Copy-Paste Discovery Prompt: US / WI / Wisconsin
...
- Save the gathered report to `reports/discovery/us-wi-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-wi-country-region-scan`
- `country`: `US`
- `region`: `WI`
...
- `"overnight summer camp" "Wisconsin"`
- `"residential summer camp" "Wisconsin"`
- `"family camp" "Wisconsin"`
- `"youth retreat" "Wisconsin" overnight`
...
- Save the JSON to `reports/discovery/us-wi-country-region-scan.json` if you can write files.
