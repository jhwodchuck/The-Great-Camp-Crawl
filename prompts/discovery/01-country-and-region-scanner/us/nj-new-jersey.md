# Copy-Paste Discovery Prompt: US / NJ / New Jersey
...
- Save the gathered report to `reports/discovery/us-nj-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-nj-country-region-scan`
- `country`: `US`
- `region`: `NJ`
...
- `"overnight summer camp" "New Jersey"`
- `"residential summer camp" "New Jersey"`
- `"family camp" "New Jersey"`
- `"youth retreat" "New Jersey" overnight`
...
- Save the JSON to `reports/discovery/us-nj-country-region-scan.json` if you can write files.
