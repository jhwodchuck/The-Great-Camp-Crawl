# Copy-Paste Discovery Prompt: US / OH / Ohio
...
- Save the gathered report to `reports/discovery/us-oh-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-oh-country-region-scan`
- `country`: `US`
- `region`: `OH`
- `region_name`: `Ohio`
...
- `"overnight summer camp" "Ohio"`
- `"residential summer camp" "Ohio"`
- `"family camp" "Ohio"`
- `"youth retreat" "Ohio" overnight`
...
- Save the JSON to `reports/discovery/us-oh-country-region-scan.json` if you can write files.
