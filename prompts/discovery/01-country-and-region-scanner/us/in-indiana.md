# Copy-Paste Discovery Prompt: US / IN / Indiana
...
- Save the gathered report to `reports/discovery/us-in-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-in-country-region-scan`
- `country`: `US`
- `region`: `IN`
- `region_name`: `Indiana`
...
- `"overnight summer camp" "Indiana"`
- `"residential summer camp" "Indiana"`
- `"family camp" "Indiana"`
- `"youth retreat" "Indiana" overnight`
...
- Save the JSON to `reports/discovery/us-in-country-region-scan.json` if you can write files.
