# Copy-Paste Discovery Prompt: US / FL / Florida
...
- Save the gathered report to `reports/discovery/us-fl-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-fl-country-region-scan`
- `country`: `US`
- `region`: `FL`
- `region_name`: `Florida`
...
- `"overnight summer camp" "Florida"`
- `"residential summer camp" "Florida"`
- `"family camp" "Florida"`
- `"youth retreat" "Florida" overnight`
...
- Save the JSON to `reports/discovery/us-fl-country-region-scan.json` if you can write files.
