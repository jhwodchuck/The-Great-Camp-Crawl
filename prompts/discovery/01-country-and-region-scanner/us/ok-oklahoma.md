# Copy-Paste Discovery Prompt: US / OK / Oklahoma
...
- Save the gathered report to `reports/discovery/us-ok-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ok-country-region-scan`
- `country`: `US`
- `region`: `OK`
...
- `"overnight summer camp" "Oklahoma"`
- `"residential summer camp" "Oklahoma"`
- `"family camp" "Oklahoma"`
- `"youth retreat" "Oklahoma" overnight`
...
- Save the JSON to `reports/discovery/us-ok-country-region-scan.json` if you can write files.
