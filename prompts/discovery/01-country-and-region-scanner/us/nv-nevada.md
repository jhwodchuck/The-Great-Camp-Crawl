# Copy-Paste Discovery Prompt: US / NV / Nevada
...
- Save the gathered report to `reports/discovery/us-nv-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-nv-country-region-scan`
- `country`: `US`
- `region`: `NV`
- `region_name`: `Nevada`
...
- `"overnight summer camp" "Nevada"`
- `"residential summer camp" "Nevada"`
- `"family camp" "Nevada"`
- `"youth retreat" "Nevada" overnight`
...
- Save the JSON to `reports/discovery/us-nv-country-region-scan.json` if you can write files.
