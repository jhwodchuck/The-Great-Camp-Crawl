# Copy-Paste Discovery Prompt: US / DC / District of Columbia
...
- Save the gathered report to `reports/discovery/us-dc-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-dc-country-region-scan`
- `country`: `US`
- `region`: `DC`
- `region_name`: `District of Columbia`
- `language_mode`: `english-first; keep Spanish and French evidence when encountered`
- `source_focus`: `official program sites first; university pre-college pages and regional camp associations next; directories only for recall expansion`

Search angles to cover:

- `"overnight summer camp" "District of Columbia"`
- `"residential summer camp" "District of Columbia"`
- `"family camp" "District of Columbia"`
- `"youth retreat" "District of Columbia" overnight`
...
- Save the JSON to `reports/discovery/us-dc-country-region-scan.json` if you can write files.
