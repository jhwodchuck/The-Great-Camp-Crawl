# Copy-Paste Discovery Prompt: US / MO / Missouri
...
- Save the gathered report to `reports/discovery/us-mo-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-mo-country-region-scan`
- `country`: `US`
- `region`: `MO`
...
- `"overnight summer camp" "Missouri"`
- `"residential summer camp" "Missouri"`
- `"family camp" "Missouri"`
- `"youth retreat" "Missouri" overnight`
...
- Save the JSON to `reports/discovery/us-mo-country-region-scan.json` if you can write files.
