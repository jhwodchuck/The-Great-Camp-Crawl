# Copy-Paste Discovery Prompt: US / WA / Washington
...
- Save the gathered report to `reports/discovery/us-wa-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-wa-country-region-scan`
- `country`: `US`
- `region`: `WA`
...
- `"overnight summer camp" "Washington"`
- `"residential summer camp" "Washington"`
- `"family camp" "Washington"`
- `"youth retreat" "Washington" overnight`
...
- Save the JSON to `reports/discovery/us-wa-country-region-scan.json` if you can write files.
