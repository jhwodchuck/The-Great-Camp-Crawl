# Copy-Paste Discovery Prompt: US / MT / Montana
...
- Save the gathered report to `reports/discovery/us-mt-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-mt-country-region-scan`
- `country`: `US`
- `region`: `MT`
- `region_name`: `Montana`
...
- `"overnight summer camp" "Montana"`
- `"residential summer camp" "Montana"`
- `"family camp" "Montana"`
- `"youth retreat" "Montana" overnight`
...
- Save the JSON to `reports/discovery/us-mt-country-region-scan.json` if you can write files.
