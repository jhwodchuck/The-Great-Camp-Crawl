# Copy-Paste Discovery Prompt: US / WY / Wyoming
...
- Save the gathered report to `reports/discovery/us-wy-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-wy-country-region-scan`
- `country`: `US`
- `region`: `WY`
- `region_name`: `Wyoming`
...
- `"overnight summer camp" "Wyoming"`
- `"residential summer camp" "Wyoming"`
- `"family camp" "Wyoming"`
- `"youth retreat" "Wyoming" overnight`
...
- Save the JSON to `reports/discovery/us-wy-country-region-scan.json` if you can write files.
