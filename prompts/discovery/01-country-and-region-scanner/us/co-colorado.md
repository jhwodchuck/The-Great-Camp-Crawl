# Copy-Paste Discovery Prompt: US / CO / Colorado
...
- Save the gathered report to `reports/discovery/us-co-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-co-country-region-scan`
- `country`: `US`
- `region`: `CO`
...
- `"overnight summer camp" "Colorado"`
- `"residential summer camp" "Colorado"`
- `"family camp" "Colorado"`
- `"youth retreat" "Colorado" overnight`
...
- Save the JSON to `reports/discovery/us-co-country-region-scan.json` if you can write files.
