# Copy-Paste Discovery Prompt: US / OR / Oregon
...
- Save the gathered report to `reports/discovery/us-or-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-or-country-region-scan`
- `country`: `US`
- `region`: `OR`
- `region_name`: `Oregon`
...
- `"overnight summer camp" "Oregon"`
- `"residential summer camp" "Oregon"`
- `"family camp" "Oregon"`
- `"youth retreat" "Oregon" overnight`
...
- Save the JSON to `reports/discovery/us-or-country-region-scan.json` if you can write files.
