# Copy-Paste Discovery Prompt: US / MN / Minnesota
...
- Save the gathered report to `reports/discovery/us-mn-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-mn-country-region-scan`
- `country`: `US`
- `region`: `MN`
...
- `"overnight summer camp" "Minnesota"`
- `"residential summer camp" "Minnesota"`
- `"family camp" "Minnesota"`
- `"youth retreat" "Minnesota" overnight`
...
- Save the JSON to `reports/discovery/us-mn-country-region-scan.json` if you can write files.
