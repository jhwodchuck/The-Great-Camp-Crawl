# Copy-Paste Discovery Prompt: US / TX / Texas
...
- Save the gathered report to `reports/discovery/us-tx-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-tx-country-region-scan`
- `country`: `US`
- `region`: `TX`
- `region_name`: `Texas`
...
- `"overnight summer camp" "Texas"`
- `"residential summer camp" "Texas"`
- `"family camp" "Texas"`
- `"youth retreat" "Texas" overnight`
...
- Save the JSON to `reports/discovery/us-tx-country-region-scan.json` if you can write files.
