# Copy-Paste Discovery Prompt: US / VT / Vermont
...
- Save the gathered report to `reports/discovery/us-vt-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-vt-country-region-scan`
- `country`: `US`
- `region`: `VT`
- `region_name`: `Vermont`
...
- `"overnight summer camp" "Vermont"`
- `"residential summer camp" "Vermont"`
- `"family camp" "Vermont"`
- `"youth retreat" "Vermont" overnight`
...
- Save the JSON to `reports/discovery/us-vt-country-region-scan.json` if you can write files.
