# Copy-Paste Discovery Prompt: US / MS / Mississippi
...
- Save the gathered report to `reports/discovery/us-ms-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ms-country-region-scan`
- `country`: `US`
- `region`: `MS`
...
- `"overnight summer camp" "Mississippi"`
- `"residential summer camp" "Mississippi"`
- `"family camp" "Mississippi"`
- `"youth retreat" "Mississippi" overnight`
...
- Save the JSON to `reports/discovery/us-ms-country-region-scan.json` if you can write files.
