# Copy-Paste Discovery Prompt: US / AZ / Arizona
...
- Save the gathered report to `reports/discovery/us-az-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-az-country-region-scan`
- `country`: `US`
- `region`: `AZ`
- `region_name`: `Arizona`
...
- `"overnight summer camp" "Arizona"`
- `"residential summer camp" "Arizona"`
- `"family camp" "Arizona"`
- `"youth retreat" "Arizona" overnight`
...
- Save the JSON to `reports/discovery/us-az-country-region-scan.json` if you can write files.
