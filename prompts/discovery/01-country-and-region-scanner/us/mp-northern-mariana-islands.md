# Copy-Paste Discovery Prompt: US / MP / Northern Mariana Islands
...
- Save the gathered report to `reports/discovery/us-mp-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-mp-country-region-scan`
- `country`: `US`
- `region`: `MP`
...
- `"overnight summer camp" "Northern Mariana Islands"`
- `"residential summer camp" "Northern Mariana Islands"`
- `"family camp" "Northern Mariana Islands"`
- `"youth retreat" "Northern Mariana Islands" overnight`
...
- Save the JSON to `reports/discovery/us-mp-country-region-scan.json` if you can write files.
