# Copy-Paste Discovery Prompt: US / CT / Connecticut
...
- Save the gathered report to `reports/discovery/us-ct-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ct-country-region-scan`
- `country`: `US`
- `region`: `CT`
...
- `"overnight summer camp" "Connecticut"`
- `"residential summer camp" "Connecticut"`
- `"family camp" "Connecticut"`
- `"youth retreat" "Connecticut" overnight`
...
- Save the JSON to `reports/discovery/us-ct-country-region-scan.json` if you can write files.
