# Copy-Paste Discovery Prompt: US / NY / New York
...
- Save the gathered report to `reports/discovery/us-ny-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ny-country-region-scan`
- `country`: `US`
- `region`: `NY`
...
- `"overnight summer camp" "New York"`
- `"residential summer camp" "New York"`
- `"family camp" "New York"`
- `"youth retreat" "New York" overnight`
...
- Save the JSON to `reports/discovery/us-ny-country-region-scan.json` if you can write files.
