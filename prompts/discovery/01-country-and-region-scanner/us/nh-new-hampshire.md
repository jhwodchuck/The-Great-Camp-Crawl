# Copy-Paste Discovery Prompt: US / NH / New Hampshire
...
- Save the gathered report to `reports/discovery/us-nh-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-nh-country-region-scan`
- `country`: `US`
- `region`: `NH`
...
- `"overnight summer camp" "New Hampshire"`
- `"residential summer camp" "New Hampshire"`
- `"family camp" "New Hampshire"`
- `"youth retreat" "New Hampshire" overnight`
...
- Save the JSON to `reports/discovery/us-nh-country-region-scan.json` if you can write files.
