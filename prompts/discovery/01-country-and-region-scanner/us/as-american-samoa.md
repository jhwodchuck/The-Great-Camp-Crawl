# Copy-Paste Discovery Prompt: US / AS / American Samoa
...
- Save the gathered report to `reports/discovery/us-as-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-as-country-region-scan`
- `country`: `US`
- `region`: `AS`
...
- `"overnight summer camp" "American Samoa"`
- `"residential summer camp" "American Samoa"`
- `"family camp" "American Samoa"`
- `"youth retreat" "American Samoa" overnight`
...
- Save the JSON to `reports/discovery/us-as-country-region-scan.json` if you can write files.
