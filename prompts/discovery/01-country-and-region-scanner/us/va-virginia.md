# Copy-Paste Discovery Prompt: US / VA / Virginia
...
- Save the gathered report to `reports/discovery/us-va-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-va-country-region-scan`
- `country`: `US`
- `region`: `VA`
...
- `"overnight summer camp" "Virginia"`
- `"residential summer camp" "Virginia"`
- `"family camp" "Virginia"`
- `"youth retreat" "Virginia" overnight`
...
- Save the JSON to `reports/discovery/us-va-country-region-scan.json` if you can write files.
