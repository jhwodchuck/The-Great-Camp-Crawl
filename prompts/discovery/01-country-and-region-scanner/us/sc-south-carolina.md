# Copy-Paste Discovery Prompt: US / SC / South Carolina
...
- Save the gathered report to `reports/discovery/us-sc-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-sc-country-region-scan`
- `country`: `US`
- `region`: `SC`
...
- `"overnight summer camp" "South Carolina"`
- `"residential summer camp" "South Carolina"`
- `"family camp" "South Carolina"`
- `"youth retreat" "South Carolina" overnight`
...
- Save the JSON to `reports/discovery/us-sc-country-region-scan.json` if you can write files.
