# Copy-Paste Discovery Prompt: US / LA / Louisiana
...
- Save the gathered report to `reports/discovery/us-la-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-la-country-region-scan`
- `country`: `US`
- `region`: `LA`
...
- `"overnight summer camp" "Louisiana"`
- `"residential summer camp" "Louisiana"`
- `"family camp" "Louisiana"`
- `"youth retreat" "Louisiana" overnight`
...
- Save the JSON to `reports/discovery/us-la-country-region-scan.json` if you can write files.
