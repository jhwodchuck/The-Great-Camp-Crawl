# Copy-Paste Discovery Prompt: US / ND / North Dakota
...
- Save the gathered report to `reports/discovery/us-nd-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-nd-country-region-scan`
- `country`: `US`
- `region`: `ND`
...
- `"overnight summer camp" "North Dakota"`
- `"residential summer camp" "North Dakota"`
- `"family camp" "North Dakota"`
- `"youth retreat" "North Dakota" overnight`
...
- Save the JSON to `reports/discovery/us-nd-country-region-scan.json` if you can write files.
