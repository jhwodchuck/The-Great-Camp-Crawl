# Copy-Paste Discovery Prompt: US / CA / California
...
- Save the gathered report to `reports/discovery/us-ca-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ca-country-region-scan`
- `country`: `US`
- `region`: `CA`
...
- `"overnight summer camp" "California"`
- `"residential summer camp" "California"`
- `"family camp" "California"`
- `"youth retreat" "California" overnight`
...
- Save the JSON to `reports/discovery/us-ca-country-region-scan.json` if you can write files.
