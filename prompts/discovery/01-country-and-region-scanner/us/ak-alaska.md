# Copy-Paste Discovery Prompt: US / AK / Alaska
...
- Save the gathered report to `reports/discovery/us-ak-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ak-country-region-scan`
- `country`: `US`
- `region`: `AK`
- `region_name`: `Alaska`
...
- `"overnight summer camp" "Alaska"`
- `"residential summer camp" "Alaska"`
- `"family camp" "Alaska"`
- `"youth retreat" "Alaska" overnight`
...
- Save the JSON to `reports/discovery/us-ak-country-region-scan.json` if you can write files.
