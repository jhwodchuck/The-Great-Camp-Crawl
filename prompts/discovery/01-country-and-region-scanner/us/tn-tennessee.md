# Copy-Paste Discovery Prompt: US / TN / Tennessee
...
- Save the gathered report to `reports/discovery/us-tn-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-tn-country-region-scan`
- `country`: `US`
- `region`: `TN`
...
- `"overnight summer camp" "Tennessee"`
- `"residential summer camp" "Tennessee"`
- `"family camp" "Tennessee"`
- `"youth retreat" "Tennessee" overnight`
...
- Save the JSON to `reports/discovery/us-tn-country-region-scan.json` if you can write files.
