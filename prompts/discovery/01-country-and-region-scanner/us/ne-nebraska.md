# Copy-Paste Discovery Prompt: US / NE / Nebraska
...
- Save the gathered report to `reports/discovery/us-ne-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ne-country-region-scan`
- `country`: `US`
- `region`: `NE`
...
- `"overnight summer camp" "Nebraska"`
- `"residential summer camp" "Nebraska"`
- `"family camp" "Nebraska"`
- `"youth retreat" "Nebraska" overnight`
...
- Save the JSON to `reports/discovery/us-ne-country-region-scan.json` if you can write files.
