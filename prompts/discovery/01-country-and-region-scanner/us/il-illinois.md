# Copy-Paste Discovery Prompt: US / IL / Illinois
...
- Save the gathered report to `reports/discovery/us-il-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-il-country-region-scan`
- `country`: `US`
- `region`: `IL`
...
- `"overnight summer camp" "Illinois"`
- `"residential summer camp" "Illinois"`
- `"family camp" "Illinois"`
- `"youth retreat" "Illinois" overnight`
...
- Save the JSON to `reports/discovery/us-il-country-region-scan.json` if you can write files.
