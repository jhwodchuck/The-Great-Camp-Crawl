# Copy-Paste Discovery Prompt: US / IA / Iowa
...
- Save the gathered report to `reports/discovery/us-ia-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ia-country-region-scan`
- `country`: `US`
- `region`: `IA`
- `region_name`: `Iowa`
...
- `"overnight summer camp" "Iowa"`
- `"residential summer camp" "Iowa"`
- `"family camp" "Iowa"`
- `"youth retreat" "Iowa" overnight`
...
- Save the JSON to `reports/discovery/us-ia-country-region-scan.json` if you can write files.
