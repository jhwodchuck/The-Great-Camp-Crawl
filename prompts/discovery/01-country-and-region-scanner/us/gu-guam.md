# Copy-Paste Discovery Prompt: US / GU / Guam
...
- Save the gathered report to `reports/discovery/us-gu-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-gu-country-region-scan`
- `country`: `US`
- `region`: `GU`
- `region_name`: `Guam`
...
- `"overnight summer camp" "Guam"`
- `"residential summer camp" "Guam"`
- `"family camp" "Guam"`
- `"youth retreat" "Guam" overnight`
...
- Save the JSON to `reports/discovery/us-gu-country-region-scan.json` if you can write files.
