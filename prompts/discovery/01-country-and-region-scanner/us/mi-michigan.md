# Copy-Paste Discovery Prompt: US / MI / Michigan
...
- Save the gathered report to `reports/discovery/us-mi-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-mi-country-region-scan`
- `country`: `US`
- `region`: `MI`
...
- `"overnight summer camp" "Michigan"`
- `"residential summer camp" "Michigan"`
- `"family camp" "Michigan"`
- `"youth retreat" "Michigan" overnight`
...
- Save the JSON to `reports/discovery/us-mi-country-region-scan.json` if you can write files.
