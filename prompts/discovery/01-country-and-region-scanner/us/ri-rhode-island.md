# Copy-Paste Discovery Prompt: US / RI / Rhode Island
...
- Save the gathered report to `reports/discovery/us-ri-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ri-country-region-scan`
- `country`: `US`
- `region`: `RI`
...
- `"overnight summer camp" "Rhode Island"`
- `"residential summer camp" "Rhode Island"`
- `"family camp" "Rhode Island"`
- `"youth retreat" "Rhode Island" overnight`
...
- Save the JSON to `reports/discovery/us-ri-country-region-scan.json` if you can write files.
