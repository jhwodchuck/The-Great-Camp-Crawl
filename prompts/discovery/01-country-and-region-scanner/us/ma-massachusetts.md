# Copy-Paste Discovery Prompt: US / MA / Massachusetts
...
- Save the gathered report to `reports/discovery/us-ma-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ma-country-region-scan`
- `country`: `US`
- `region`: `MA`
...
- `"overnight summer camp" "Massachusetts"`
- `"residential summer camp" "Massachusetts"`
- `"family camp" "Massachusetts"`
- `"youth retreat" "Massachusetts" overnight`
...
- Save the JSON to `reports/discovery/us-ma-country-region-scan.json` if you can write files.
