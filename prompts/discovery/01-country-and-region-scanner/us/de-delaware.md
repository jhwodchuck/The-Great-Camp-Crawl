# Copy-Paste Discovery Prompt: US / DE / Delaware
...
- Save the gathered report to `reports/discovery/us-de-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-de-country-region-scan`
- `country`: `US`
- `region`: `DE`
...
- `"overnight summer camp" "Delaware"`
- `"residential summer camp" "Delaware"`
- `"family camp" "Delaware"`
- `"youth retreat" "Delaware" overnight`
...
- Save the JSON to `reports/discovery/us-de-country-region-scan.json` if you can write files.
