# Copy-Paste Discovery Prompt: US / HI / Hawaii
...
- Save the gathered report to `reports/discovery/us-hi-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-hi-country-region-scan`
- `country`: `US`
- `region`: `HI`
- `region_name`: `Hawaii`
...
- `"overnight summer camp" "Hawaii"`
- `"residential summer camp" "Hawaii"`
- `"family camp" "Hawaii"`
- `"youth retreat" "Hawaii" overnight`
...
- Save the JSON to `reports/discovery/us-hi-country-region-scan.json` if you can write files.
