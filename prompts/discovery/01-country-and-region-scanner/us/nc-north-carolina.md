# Copy-Paste Discovery Prompt: US / NC / North Carolina
...
- Save the gathered report to `reports/discovery/us-nc-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-nc-country-region-scan`
- `country`: `US`
- `region`: `NC`
...
- `"overnight summer camp" "North Carolina"`
- `"residential summer camp" "North Carolina"`
- `"family camp" "North Carolina"`
- `"youth retreat" "North Carolina" overnight`
...
- Save the JSON to `reports/discovery/us-nc-country-region-scan.json` if you can write files.
