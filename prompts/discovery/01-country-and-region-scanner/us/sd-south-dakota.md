# Copy-Paste Discovery Prompt: US / SD / South Dakota
...
- Save the gathered report to `reports/discovery/us-sd-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-sd-country-region-scan`
- `country`: `US`
- `region`: `SD`
...
- `"overnight summer camp" "South Dakota"`
- `"residential summer camp" "South Dakota"`
- `"family camp" "South Dakota"`
- `"youth retreat" "South Dakota" overnight`
...
- Save the JSON to `reports/discovery/us-sd-country-region-scan.json` if you can write files.
