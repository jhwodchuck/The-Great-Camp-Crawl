# Copy-Paste Discovery Prompt: US / ID / Idaho
...
- Save the gathered report to `reports/discovery/us-id-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-id-country-region-scan`
- `country`: `US`
- `region`: `ID`
- `region_name`: `Idaho`
...
- `"overnight summer camp" "Idaho"`
- `"residential summer camp" "Idaho"`
- `"family camp" "Idaho"`
- `"youth retreat" "Idaho" overnight`
...
- Save the JSON to `reports/discovery/us-id-country-region-scan.json` if you can write files.
