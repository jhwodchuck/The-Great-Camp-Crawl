# Copy-Paste Discovery Prompt: US / PR / Puerto Rico
...
- Save the gathered report to `reports/discovery/us-pr-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-pr-country-region-scan`
- `country`: `US`
- `region`: `PR`
...
- `"overnight summer camp" "Puerto Rico"`
- `"residential summer camp" "Puerto Rico"`
- `"family camp" "Puerto Rico"`
- `"youth retreat" "Puerto Rico" overnight`
...
- Save the JSON to `reports/discovery/us-pr-country-region-scan.json` if you can write files.
