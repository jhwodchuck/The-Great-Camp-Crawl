# Copy-Paste Discovery Prompt: US / GA / Georgia
...
- Save the gathered report to `reports/discovery/us-ga-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ga-country-region-scan`
- `country`: `US`
- `region`: `GA`
- `region_name`: `Georgia`
...
- `"overnight summer camp" "Georgia"`
- `"residential summer camp" "Georgia"`
- `"family camp" "Georgia"`
- `"youth retreat" "Georgia" overnight`
...
- Save the JSON to `reports/discovery/us-ga-country-region-scan.json` if you can write files.
