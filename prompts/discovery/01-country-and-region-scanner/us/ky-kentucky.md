# Copy-Paste Discovery Prompt: US / KY / Kentucky
...
- Save the gathered report to `reports/discovery/us-ky-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ky-country-region-scan`
- `country`: `US`
- `region`: `KY`
...
- `"overnight summer camp" "Kentucky"`
- `"residential summer camp" "Kentucky"`
- `"family camp" "Kentucky"`
- `"youth retreat" "Kentucky" overnight`
...
- Save the JSON to `reports/discovery/us-ky-country-region-scan.json` if you can write files.
