# Copy-Paste Discovery Prompt: US / NM / New Mexico
...
- Save the gathered report to `reports/discovery/us-nm-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-nm-country-region-scan`
- `country`: `US`
- `region`: `NM`
...
- `"overnight summer camp" "New Mexico"`
- `"residential summer camp" "New Mexico"`
- `"family camp" "New Mexico"`
- `"youth retreat" "New Mexico" overnight`
...
- Save the JSON to `reports/discovery/us-nm-country-region-scan.json` if you can write files.
