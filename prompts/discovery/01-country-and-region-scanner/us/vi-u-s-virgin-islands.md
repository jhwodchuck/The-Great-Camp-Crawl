# Copy-Paste Discovery Prompt: US / VI / U.S. Virgin Islands
...
- Save the gathered report to `reports/discovery/us-vi-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-vi-country-region-scan`
- `country`: `US`
- `region`: `VI`
...
- `"overnight summer camp" "U.S. Virgin Islands"`
- `"residential summer camp" "U.S. Virgin Islands"`
- `"family camp" "U.S. Virgin Islands"`
- `"youth retreat" "U.S. Virgin Islands" overnight`
...
- Save the JSON to `reports/discovery/us-vi-country-region-scan.json` if you can write files.
