# Copy-Paste Discovery Prompt: US / UT / Utah
...
- Save the gathered report to `reports/discovery/us-ut-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ut-country-region-scan`
- `country`: `US`
- `region`: `UT`
- `region_name`: `Utah`
...
- `"overnight summer camp" "Utah"`
- `"residential summer camp" "Utah"`
- `"family camp" "Utah"`
- `"youth retreat" "Utah" overnight`
...
- Save the JSON to `reports/discovery/us-ut-country-region-scan.json` if you can write files.
