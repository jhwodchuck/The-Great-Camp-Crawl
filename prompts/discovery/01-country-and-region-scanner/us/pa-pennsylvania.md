# Copy-Paste Discovery Prompt: US / PA / Pennsylvania
...
- Save the gathered report to `reports/discovery/us-pa-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-pa-country-region-scan`
- `country`: `US`
- `region`: `PA`
...
- `"overnight summer camp" "Pennsylvania"`
- `"residential summer camp" "Pennsylvania"`
- `"family camp" "Pennsylvania"`
- `"youth retreat" "Pennsylvania" overnight`
...
- Save the JSON to `reports/discovery/us-pa-country-region-scan.json` if you can write files.
