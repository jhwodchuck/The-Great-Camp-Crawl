# Copy-Paste Discovery Prompt: US / WV / West Virginia
...
- Save the gathered report to `reports/discovery/us-wv-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-wv-country-region-scan`
- `country`: `US`
- `region`: `WV`
...
- `"overnight summer camp" "West Virginia"`
- `"residential summer camp" "West Virginia"`
- `"family camp" "West Virginia"`
- `"youth retreat" "West Virginia" overnight`
...
- Save the JSON to `reports/discovery/us-wv-country-region-scan.json` if you can write files.
