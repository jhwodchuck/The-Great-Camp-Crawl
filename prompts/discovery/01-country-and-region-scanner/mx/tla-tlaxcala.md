# Copy-Paste Discovery Prompt: MX / TLA / Tlaxcala
...
- Save the gathered report to `reports/discovery/mx-tla-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-tla-country-region-scan`
- `country`: `MX`
- `region`: `TLA`
...
- `"campamento de verano con alojamiento" "Tlaxcala"`
- `"campamento residencial" "Tlaxcala"`
- `"campamento juvenil" "Tlaxcala"`
- `"retiro juvenil" "Tlaxcala" hospedaje`
- `site:.mx "Tlaxcala" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-tla-country-region-scan.json` if you can write files.
