# Copy-Paste Discovery Prompt: MX / DUR / Durango
...
- Save the gathered report to `reports/discovery/mx-dur-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-dur-country-region-scan`
- `country`: `MX`
- `region`: `DUR`
- `region_name`: `Durango`
...
- `"campamento de verano con alojamiento" "Durango"`
- `"campamento residencial" "Durango"`
- `"campamento juvenil" "Durango"`
- `"retiro juvenil" "Durango" hospedaje`
- `site:.mx "Durango" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-dur-country-region-scan.json` if you can write files.
