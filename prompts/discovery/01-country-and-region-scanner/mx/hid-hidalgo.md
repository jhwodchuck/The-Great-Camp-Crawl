# Copy-Paste Discovery Prompt: MX / HID / Hidalgo
...
- Save the gathered report to `reports/discovery/mx-hid-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-hid-country-region-scan`
- `country`: `MX`
- `region`: `HID`
- `region_name`: `Hidalgo`
...
- `"campamento de verano con alojamiento" "Hidalgo"`
- `"campamento residencial" "Hidalgo"`
- `"campamento juvenil" "Hidalgo"`
- `"retiro juvenil" "Hidalgo" hospedaje`
- `site:.mx "Hidalgo" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-hid-country-region-scan.json` if you can write files.
