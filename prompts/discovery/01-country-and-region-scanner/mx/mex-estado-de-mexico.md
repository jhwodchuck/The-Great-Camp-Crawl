# Copy-Paste Discovery Prompt: MX / MEX / Estado de Mexico
...
- Save the gathered report to `reports/discovery/mx-mex-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-mex-country-region-scan`
- `country`: `MX`
- `region`: `MEX`
- `region_name`: `Estado de Mexico`
- `language_mode`: `spanish-first; keep English evidence when encountered`
- `source_focus`: `official program sites first; university and regional program pages next; directories only for recall expansion`

Search angles to cover:

- `"campamento de verano con alojamiento" "Estado de Mexico"`
- `"campamento residencial" "Estado de Mexico"`
- `"campamento juvenil" "Estado de Mexico"`
- `"retiro juvenil" "Estado de Mexico" hospedaje`
- `site:.mx "Estado de Mexico" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-mex-country-region-scan.json` if you can write files.
