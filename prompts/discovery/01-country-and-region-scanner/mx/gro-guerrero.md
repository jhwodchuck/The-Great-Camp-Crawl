# Copy-Paste Discovery Prompt: MX / GRO / Guerrero
...
- Save the gathered report to `reports/discovery/mx-gro-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-gro-country-region-scan`
- `country`: `MX`
- `region`: `GRO`
...
- `"campamento de verano con alojamiento" "Guerrero"`
- `"campamento residencial" "Guerrero"`
- `"campamento juvenil" "Guerrero"`
- `"retiro juvenil" "Guerrero" hospedaje`
- `site:.mx "Guerrero" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-gro-country-region-scan.json` if you can write files.
