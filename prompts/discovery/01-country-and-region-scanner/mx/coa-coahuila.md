# Copy-Paste Discovery Prompt: MX / COA / Coahuila
...
- Save the gathered report to `reports/discovery/mx-coa-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-coa-country-region-scan`
- `country`: `MX`
- `region`: `COA`
...
- `"campamento de verano con alojamiento" "Coahuila"`
- `"campamento residencial" "Coahuila"`
- `"campamento juvenil" "Coahuila"`
- `"retiro juvenil" "Coahuila" hospedaje`
- `site:.mx "Coahuila" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-coa-country-region-scan.json` if you can write files.
