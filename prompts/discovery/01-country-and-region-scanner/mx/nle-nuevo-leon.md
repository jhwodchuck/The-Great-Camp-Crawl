# Copy-Paste Discovery Prompt: MX / NLE / Nuevo Leon
...
- Save the gathered report to `reports/discovery/mx-nle-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-nle-country-region-scan`
- `country`: `MX`
- `region`: `NLE`
...
- `"campamento de verano con alojamiento" "Nuevo Leon"`
- `"campamento residencial" "Nuevo Leon"`
- `"campamento juvenil" "Nuevo Leon"`
- `"retiro juvenil" "Nuevo Leon" hospedaje`
- `site:.mx "Nuevo Leon" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-nle-country-region-scan.json` if you can write files.
