# Copy-Paste Discovery Prompt: MX / MOR / Morelos
...
- Save the gathered report to `reports/discovery/mx-mor-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-mor-country-region-scan`
- `country`: `MX`
- `region`: `MOR`
- `region_name`: `Morelos`
...
- `"campamento de verano con alojamiento" "Morelos"`
- `"campamento residencial" "Morelos"`
- `"campamento juvenil" "Morelos"`
- `"retiro juvenil" "Morelos" hospedaje`
- `site:.mx "Morelos" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-mor-country-region-scan.json` if you can write files.
