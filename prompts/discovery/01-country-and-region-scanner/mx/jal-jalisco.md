# Copy-Paste Discovery Prompt: MX / JAL / Jalisco
...
- Save the gathered report to `reports/discovery/mx-jal-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-jal-country-region-scan`
- `country`: `MX`
- `region`: `JAL`
- `region_name`: `Jalisco`
...
- `"campamento de verano con alojamiento" "Jalisco"`
- `"campamento residencial" "Jalisco"`
- `"campamento juvenil" "Jalisco"`
- `"retiro juvenil" "Jalisco" hospedaje`
- `site:.mx "Jalisco" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-jal-country-region-scan.json` if you can write files.
