# Copy-Paste Discovery Prompt: MX / SLP / San Luis Potosi
...
- Save the gathered report to `reports/discovery/mx-slp-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-slp-country-region-scan`
- `country`: `MX`
- `region`: `SLP`
...
- `"campamento de verano con alojamiento" "San Luis Potosi"`
- `"campamento residencial" "San Luis Potosi"`
- `"campamento juvenil" "San Luis Potosi"`
- `"retiro juvenil" "San Luis Potosi" hospedaje`
- `site:.mx "San Luis Potosi" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-slp-country-region-scan.json` if you can write files.
