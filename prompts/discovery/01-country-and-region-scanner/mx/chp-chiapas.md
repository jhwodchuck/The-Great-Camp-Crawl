# Copy-Paste Discovery Prompt: MX / CHP / Chiapas
...
- Save the gathered report to `reports/discovery/mx-chp-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-chp-country-region-scan`
- `country`: `MX`
- `region`: `CHP`
- `region_name`: `Chiapas`
...
- `"campamento de verano con alojamiento" "Chiapas"`
- `"campamento residencial" "Chiapas"`
- `"campamento juvenil" "Chiapas"`
- `"retiro juvenil" "Chiapas" hospedaje`
- `site:.mx "Chiapas" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-chp-country-region-scan.json` if you can write files.
