# Copy-Paste Discovery Prompt: MX / CHH / Chihuahua
...
- Save the gathered report to `reports/discovery/mx-chh-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-chh-country-region-scan`
- `country`: `MX`
- `region`: `CHH`
...
- `"campamento de verano con alojamiento" "Chihuahua"`
- `"campamento residencial" "Chihuahua"`
- `"campamento juvenil" "Chihuahua"`
- `"retiro juvenil" "Chihuahua" hospedaje`
- `site:.mx "Chihuahua" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-chh-country-region-scan.json` if you can write files.
