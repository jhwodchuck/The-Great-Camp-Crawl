# Copy-Paste Discovery Prompt: MX / MIC / Michoacan
...
- Save the gathered report to `reports/discovery/mx-mic-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-mic-country-region-scan`
- `country`: `MX`
- `region`: `MIC`
...
- `"campamento de verano con alojamiento" "Michoacan"`
- `"campamento residencial" "Michoacan"`
- `"campamento juvenil" "Michoacan"`
- `"retiro juvenil" "Michoacan" hospedaje`
- `site:.mx "Michoacan" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-mic-country-region-scan.json` if you can write files.
