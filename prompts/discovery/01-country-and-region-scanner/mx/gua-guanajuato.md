# Copy-Paste Discovery Prompt: MX / GUA / Guanajuato
...
- Save the gathered report to `reports/discovery/mx-gua-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-gua-country-region-scan`
- `country`: `MX`
- `region`: `GUA`
...
- `"campamento de verano con alojamiento" "Guanajuato"`
- `"campamento residencial" "Guanajuato"`
- `"campamento juvenil" "Guanajuato"`
- `"retiro juvenil" "Guanajuato" hospedaje`
- `site:.mx "Guanajuato" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-gua-country-region-scan.json` if you can write files.
