# Copy-Paste Discovery Prompt: MX / VER / Veracruz
...
- Save the gathered report to `reports/discovery/mx-ver-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-ver-country-region-scan`
- `country`: `MX`
- `region`: `VER`
...
- `"campamento de verano con alojamiento" "Veracruz"`
- `"campamento residencial" "Veracruz"`
- `"campamento juvenil" "Veracruz"`
- `"retiro juvenil" "Veracruz" hospedaje`
- `site:.mx "Veracruz" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-ver-country-region-scan.json` if you can write files.
