# Copy-Paste Discovery Prompt: MX / BCS / Baja California Sur
...
- Save the gathered report to `reports/discovery/mx-bcs-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-bcs-country-region-scan`
- `country`: `MX`
- `region`: `BCS`
...
- `"campamento de verano con alojamiento" "Baja California Sur"`
- `"campamento residencial" "Baja California Sur"`
- `"campamento juvenil" "Baja California Sur"`
- `"retiro juvenil" "Baja California Sur" hospedaje`
- `site:.mx "Baja California Sur" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-bcs-country-region-scan.json` if you can write files.
