# Copy-Paste Discovery Prompt: MX / CAM / Campeche
...
- Save the gathered report to `reports/discovery/mx-cam-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-cam-country-region-scan`
- `country`: `MX`
- `region`: `CAM`
...
- `"campamento de verano con alojamiento" "Campeche"`
- `"campamento residencial" "Campeche"`
- `"campamento juvenil" "Campeche"`
- `"retiro juvenil" "Campeche" hospedaje`
- `site:.mx "Campeche" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-cam-country-region-scan.json` if you can write files.
