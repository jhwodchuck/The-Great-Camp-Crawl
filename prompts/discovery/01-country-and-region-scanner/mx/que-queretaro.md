# Copy-Paste Discovery Prompt: MX / QUE / Queretaro
...
- Save the gathered report to `reports/discovery/mx-que-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-que-country-region-scan`
- `country`: `MX`
- `region`: `QUE`
...
- `"campamento de verano con alojamiento" "Queretaro"`
- `"campamento residencial" "Queretaro"`
- `"campamento juvenil" "Queretaro"`
- `"retiro juvenil" "Queretaro" hospedaje`
- `site:.mx "Queretaro" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-que-country-region-scan.json` if you can write files.
