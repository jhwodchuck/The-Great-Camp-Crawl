# Copy-Paste Discovery Prompt: MX / PUE / Puebla
...
- Save the gathered report to `reports/discovery/mx-pue-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-pue-country-region-scan`
- `country`: `MX`
- `region`: `PUE`
- `region_name`: `Puebla`
...
- `"campamento de verano con alojamiento" "Puebla"`
- `"campamento residencial" "Puebla"`
- `"campamento juvenil" "Puebla"`
- `"retiro juvenil" "Puebla" hospedaje`
- `site:.mx "Puebla" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-pue-country-region-scan.json` if you can write files.
