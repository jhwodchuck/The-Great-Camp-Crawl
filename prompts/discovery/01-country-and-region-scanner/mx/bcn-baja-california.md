# Copy-Paste Discovery Prompt: MX / BCN / Baja California
...
- Save the gathered report to `reports/discovery/mx-bcn-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-bcn-country-region-scan`
- `country`: `MX`
- `region`: `BCN`
...
- `"campamento de verano con alojamiento" "Baja California"`
- `"campamento residencial" "Baja California"`
- `"campamento juvenil" "Baja California"`
- `"retiro juvenil" "Baja California" hospedaje`
- `site:.mx "Baja California" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-bcn-country-region-scan.json` if you can write files.
