# Copy-Paste Discovery Prompt: MX / ZAC / Zacatecas
...
- Save the gathered report to `reports/discovery/mx-zac-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-zac-country-region-scan`
- `country`: `MX`
- `region`: `ZAC`
...
- `"campamento de verano con alojamiento" "Zacatecas"`
- `"campamento residencial" "Zacatecas"`
- `"campamento juvenil" "Zacatecas"`
- `"retiro juvenil" "Zacatecas" hospedaje`
- `site:.mx "Zacatecas" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-zac-country-region-scan.json` if you can write files.
