# Copy-Paste Discovery Prompt: MX / NAY / Nayarit
...
- Save the gathered report to `reports/discovery/mx-nay-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-nay-country-region-scan`
- `country`: `MX`
- `region`: `NAY`
- `region_name`: `Nayarit`
...
- `"campamento de verano con alojamiento" "Nayarit"`
- `"campamento residencial" "Nayarit"`
- `"campamento juvenil" "Nayarit"`
- `"retiro juvenil" "Nayarit" hospedaje`
- `site:.mx "Nayarit" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-nay-country-region-scan.json` if you can write files.
