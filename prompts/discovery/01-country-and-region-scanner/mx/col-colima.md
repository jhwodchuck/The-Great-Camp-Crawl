# Copy-Paste Discovery Prompt: MX / COL / Colima
...
- Save the gathered report to `reports/discovery/mx-col-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-col-country-region-scan`
- `country`: `MX`
- `region`: `COL`
- `region_name`: `Colima`
...
- `"campamento de verano con alojamiento" "Colima"`
- `"campamento residencial" "Colima"`
- `"campamento juvenil" "Colima"`
- `"retiro juvenil" "Colima" hospedaje`
- `site:.mx "Colima" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-col-country-region-scan.json` if you can write files.
