# Copy-Paste Discovery Prompt: MX / OAX / Oaxaca
...
- Save the gathered report to `reports/discovery/mx-oax-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-oax-country-region-scan`
- `country`: `MX`
- `region`: `OAX`
- `region_name`: `Oaxaca`
...
- `"campamento de verano con alojamiento" "Oaxaca"`
- `"campamento residencial" "Oaxaca"`
- `"campamento juvenil" "Oaxaca"`
- `"retiro juvenil" "Oaxaca" hospedaje`
- `site:.mx "Oaxaca" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-oax-country-region-scan.json` if you can write files.
