# Copy-Paste Discovery Prompt: MX / TAB / Tabasco
...
- Save the gathered report to `reports/discovery/mx-tab-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-tab-country-region-scan`
- `country`: `MX`
- `region`: `TAB`
- `region_name`: `Tabasco`
...
- `"campamento de verano con alojamiento" "Tabasco"`
- `"campamento residencial" "Tabasco"`
- `"campamento juvenil" "Tabasco"`
- `"retiro juvenil" "Tabasco" hospedaje`
- `site:.mx "Tabasco" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-tab-country-region-scan.json` if you can write files.
