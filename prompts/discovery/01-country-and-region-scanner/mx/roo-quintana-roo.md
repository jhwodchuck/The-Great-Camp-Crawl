# Copy-Paste Discovery Prompt: MX / ROO / Quintana Roo
...
- Save the gathered report to `reports/discovery/mx-roo-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-roo-country-region-scan`
- `country`: `MX`
- `region`: `ROO`
...
- `"campamento de verano con alojamiento" "Quintana Roo"`
- `"campamento residencial" "Quintana Roo"`
- `"campamento juvenil" "Quintana Roo"`
- `"retiro juvenil" "Quintana Roo" hospedaje`
- `site:.mx "Quintana Roo" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-roo-country-region-scan.json` if you can write files.
