# Copy-Paste Discovery Prompt: MX / TAM / Tamaulipas
...
- Save the gathered report to `reports/discovery/mx-tam-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-tam-country-region-scan`
- `country`: `MX`
- `region`: `TAM`
...
- `"campamento de verano con alojamiento" "Tamaulipas"`
- `"campamento residencial" "Tamaulipas"`
- `"campamento juvenil" "Tamaulipas"`
- `"retiro juvenil" "Tamaulipas" hospedaje`
- `site:.mx "Tamaulipas" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-tam-country-region-scan.json` if you can write files.
