# Copy-Paste Discovery Prompt: MX / AGU / Aguascalientes
...
- Save the gathered report to `reports/discovery/mx-agu-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-agu-country-region-scan`
- `country`: `MX`
- `region`: `AGU`
...
- `"campamento de verano con alojamiento" "Aguascalientes"`
- `"campamento residencial" "Aguascalientes"`
- `"campamento juvenil" "Aguascalientes"`
- `"retiro juvenil" "Aguascalientes" hospedaje`
- `site:.mx "Aguascalientes" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-agu-country-region-scan.json` if you can write files.
