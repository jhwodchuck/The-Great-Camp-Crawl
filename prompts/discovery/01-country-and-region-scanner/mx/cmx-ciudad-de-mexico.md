# Copy-Paste Discovery Prompt: MX / CMX / Ciudad de Mexico
...
- Save the gathered report to `reports/discovery/mx-cmx-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-cmx-country-region-scan`
- `country`: `MX`
- `region`: `CMX`
...
- `"campamento de verano con alojamiento" "Ciudad de Mexico"`
- `"campamento residencial" "Ciudad de Mexico"`
- `"campamento juvenil" "Ciudad de Mexico"`
- `"retiro juvenil" "Ciudad de Mexico" hospedaje`
- `site:.mx "Ciudad de Mexico" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-cmx-country-region-scan.json` if you can write files.
