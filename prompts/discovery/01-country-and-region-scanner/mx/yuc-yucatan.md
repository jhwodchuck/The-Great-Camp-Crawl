# Copy-Paste Discovery Prompt: MX / YUC / Yucatan
...
- Save the gathered report to `reports/discovery/mx-yuc-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-yuc-country-region-scan`
- `country`: `MX`
- `region`: `YUC`
- `region_name`: `Yucatan`
...
- `"campamento de verano con alojamiento" "Yucatan"`
- `"campamento residencial" "Yucatan"`
- `"campamento juvenil" "Yucatan"`
- `"retiro juvenil" "Yucatan" hospedaje`
- `site:.mx "Yucatan" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-yuc-country-region-scan.json` if you can write files.
