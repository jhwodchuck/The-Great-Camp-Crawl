# Copy-Paste Discovery Prompt: MX / SIN / Sinaloa
...
- Save the gathered report to `reports/discovery/mx-sin-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-sin-country-region-scan`
- `country`: `MX`
- `region`: `SIN`
- `region_name`: `Sinaloa`
...
- `"campamento de verano con alojamiento" "Sinaloa"`
- `"campamento residencial" "Sinaloa"`
- `"campamento juvenil" "Sinaloa"`
- `"retiro juvenil" "Sinaloa" hospedaje`
- `site:.mx "Sinaloa" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-sin-country-region-scan.json` if you can write files.
