# Copy-Paste Discovery Prompt: MX / SON / Sonora
...
- Save the gathered report to `reports/discovery/mx-son-country-region-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-son-country-region-scan`
- `country`: `MX`
- `region`: `SON`
- `region_name`: `Sonora`
...
- `"campamento de verano con alojamiento" "Sonora"`
- `"campamento residencial" "Sonora"`
- `"campamento juvenil" "Sonora"`
- `"retiro juvenil" "Sonora" hospedaje`
- `site:.mx "Sonora" programa residencial preuniversitario`
...
- Save the JSON to `reports/discovery/mx-son-country-region-scan.json` if you can write files.
