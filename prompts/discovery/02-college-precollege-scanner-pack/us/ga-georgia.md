# Copy-Paste Discovery Prompt: US / GA / Georgia — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ga-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ga-college-precollege-scan`
- `country`: `US`
- `region`: `GA`
- `region_name`: `Georgia`
...
- `site:.edu "Georgia" "pre-college" residential`
- `site:.edu "Georgia" "summer program" residential overnight`
- `site:.edu "Georgia" "summer immersion" OR "summer institute" residential`
- `"Georgia" university "summer camp" residential "high school"`
- `"Georgia" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ga-college-precollege-scan.json` if you can write files.
