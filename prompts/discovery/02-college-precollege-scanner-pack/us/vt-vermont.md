# Copy-Paste Discovery Prompt: US / VT / Vermont — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-vt-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-vt-college-precollege-scan`
- `country`: `US`
- `region`: `VT`
- `region_name`: `Vermont`
...
- `site:.edu "Vermont" "pre-college" residential`
- `site:.edu "Vermont" "summer program" residential overnight`
- `site:.edu "Vermont" "summer immersion" OR "summer institute" residential`
- `"Vermont" university "summer camp" residential "high school"`
- `"Vermont" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-vt-college-precollege-scan.json` if you can write files.
