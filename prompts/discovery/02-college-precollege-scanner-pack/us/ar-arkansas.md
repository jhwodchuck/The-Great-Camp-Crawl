# Copy-Paste Discovery Prompt: US / AR / Arkansas — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ar-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ar-college-precollege-scan`
- `country`: `US`
- `region`: `AR`
...
- `site:.edu "Arkansas" "pre-college" residential`
- `site:.edu "Arkansas" "summer program" residential overnight`
- `site:.edu "Arkansas" "summer immersion" OR "summer institute" residential`
- `"Arkansas" university "summer camp" residential "high school"`
- `"Arkansas" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ar-college-precollege-scan.json` if you can write files.
