# Copy-Paste Discovery Prompt: US / OK / Oklahoma — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ok-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ok-college-precollege-scan`
- `country`: `US`
- `region`: `OK`
...
- `site:.edu "Oklahoma" "pre-college" residential`
- `site:.edu "Oklahoma" "summer program" residential overnight`
- `site:.edu "Oklahoma" "summer immersion" OR "summer institute" residential`
- `"Oklahoma" university "summer camp" residential "high school"`
- `"Oklahoma" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ok-college-precollege-scan.json` if you can write files.
