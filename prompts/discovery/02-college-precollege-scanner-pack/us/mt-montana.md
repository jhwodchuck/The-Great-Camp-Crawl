# Copy-Paste Discovery Prompt: US / MT / Montana — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-mt-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-mt-college-precollege-scan`
- `country`: `US`
- `region`: `MT`
- `region_name`: `Montana`
...
- `site:.edu "Montana" "pre-college" residential`
- `site:.edu "Montana" "summer program" residential overnight`
- `site:.edu "Montana" "summer immersion" OR "summer institute" residential`
- `"Montana" university "summer camp" residential "high school"`
- `"Montana" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-mt-college-precollege-scan.json` if you can write files.
