# Copy-Paste Discovery Prompt: US / GU / Guam — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-gu-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-gu-college-precollege-scan`
- `country`: `US`
- `region`: `GU`
- `region_name`: `Guam`
...
- `site:.edu "Guam" "pre-college" residential`
- `site:.edu "Guam" "summer program" residential overnight`
- `site:.edu "Guam" "summer immersion" OR "summer institute" residential`
- `"Guam" university "summer camp" residential "high school"`
- `"Guam" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-gu-college-precollege-scan.json` if you can write files.
