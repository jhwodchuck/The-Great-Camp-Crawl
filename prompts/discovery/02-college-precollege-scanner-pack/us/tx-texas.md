# Copy-Paste Discovery Prompt: US / TX / Texas — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-tx-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-tx-college-precollege-scan`
- `country`: `US`
- `region`: `TX`
- `region_name`: `Texas`
...
- `site:.edu "Texas" "pre-college" residential`
- `site:.edu "Texas" "summer program" residential overnight`
- `site:.edu "Texas" "summer immersion" OR "summer institute" residential`
- `"Texas" university "summer camp" residential "high school"`
- `"Texas" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-tx-college-precollege-scan.json` if you can write files.
