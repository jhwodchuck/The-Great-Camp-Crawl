# Copy-Paste Discovery Prompt: US / KS / Kansas — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ks-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ks-college-precollege-scan`
- `country`: `US`
- `region`: `KS`
- `region_name`: `Kansas`
...
- `site:.edu "Kansas" "pre-college" residential`
- `site:.edu "Kansas" "summer program" residential overnight`
- `site:.edu "Kansas" "summer immersion" OR "summer institute" residential`
- `"Kansas" university "summer camp" residential "high school"`
- `"Kansas" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ks-college-precollege-scan.json` if you can write files.
