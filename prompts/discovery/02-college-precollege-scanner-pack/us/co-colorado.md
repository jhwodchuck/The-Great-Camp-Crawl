# Copy-Paste Discovery Prompt: US / CO / Colorado — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-co-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-co-college-precollege-scan`
- `country`: `US`
- `region`: `CO`
...
- `site:.edu "Colorado" "pre-college" residential`
- `site:.edu "Colorado" "summer program" residential overnight`
- `site:.edu "Colorado" "summer immersion" OR "summer institute" residential`
- `"Colorado" university "summer camp" residential "high school"`
- `"Colorado" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-co-college-precollege-scan.json` if you can write files.
