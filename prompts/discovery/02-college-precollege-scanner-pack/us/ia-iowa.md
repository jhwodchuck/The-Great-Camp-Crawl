# Copy-Paste Discovery Prompt: US / IA / Iowa — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ia-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ia-college-precollege-scan`
- `country`: `US`
- `region`: `IA`
- `region_name`: `Iowa`
...
- `site:.edu "Iowa" "pre-college" residential`
- `site:.edu "Iowa" "summer program" residential overnight`
- `site:.edu "Iowa" "summer immersion" OR "summer institute" residential`
- `"Iowa" university "summer camp" residential "high school"`
- `"Iowa" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ia-college-precollege-scan.json` if you can write files.
