# Copy-Paste Discovery Prompt: US / TN / Tennessee — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-tn-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-tn-college-precollege-scan`
- `country`: `US`
- `region`: `TN`
...
- `site:.edu "Tennessee" "pre-college" residential`
- `site:.edu "Tennessee" "summer program" residential overnight`
- `site:.edu "Tennessee" "summer immersion" OR "summer institute" residential`
- `"Tennessee" university "summer camp" residential "high school"`
- `"Tennessee" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-tn-college-precollege-scan.json` if you can write files.
