# Copy-Paste Discovery Prompt: US / NY / New York — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ny-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ny-college-precollege-scan`
- `country`: `US`
- `region`: `NY`
...
- `site:.edu "New York" "pre-college" residential`
- `site:.edu "New York" "summer program" residential overnight`
- `site:.edu "New York" "summer immersion" OR "summer institute" residential`
- `"New York" university "summer camp" residential "high school"`
- `"New York" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ny-college-precollege-scan.json` if you can write files.
