# Copy-Paste Discovery Prompt: US / RI / Rhode Island — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ri-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ri-college-precollege-scan`
- `country`: `US`
- `region`: `RI`
...
- `site:.edu "Rhode Island" "pre-college" residential`
- `site:.edu "Rhode Island" "summer program" residential overnight`
- `site:.edu "Rhode Island" "summer immersion" OR "summer institute" residential`
- `"Rhode Island" university "summer camp" residential "high school"`
- `"Rhode Island" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ri-college-precollege-scan.json` if you can write files.
