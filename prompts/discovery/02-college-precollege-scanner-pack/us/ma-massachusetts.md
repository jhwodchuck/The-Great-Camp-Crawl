# Copy-Paste Discovery Prompt: US / MA / Massachusetts — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ma-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ma-college-precollege-scan`
- `country`: `US`
- `region`: `MA`
...
- `site:.edu "Massachusetts" "summer program" residential overnight`
- `site:.edu "Massachusetts" "summer immersion" OR "summer institute" residential`
- `"Massachusetts" university "summer camp" residential "high school"`
- `"Massachusetts" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ma-college-precollege-scan.json` if you can write files.
