# Copy-Paste Discovery Prompt: US / OR / Oregon — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-or-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-or-college-precollege-scan`
- `country`: `US`
- `region`: `OR`
- `region_name`: `Oregon`
...
- `site:.edu "Oregon" "pre-college" residential`
- `site:.edu "Oregon" "summer program" residential overnight`
- `site:.edu "Oregon" "summer immersion" OR "summer institute" residential`
- `"Oregon" university "summer camp" residential "high school"`
- `"Oregon" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-or-college-precollege-scan.json` if you can write files.
