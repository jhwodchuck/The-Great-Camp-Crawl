# Copy-Paste Discovery Prompt: US / UT / Utah — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-ut-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-ut-college-precollege-scan`
- `country`: `US`
- `region`: `UT`
- `region_name`: `Utah`
...
- `site:.edu "Utah" "pre-college" residential`
- `site:.edu "Utah" "summer program" residential overnight`
- `site:.edu "Utah" "summer immersion" OR "summer institute" residential`
- `"Utah" university "summer camp" residential "high school"`
- `"Utah" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-ut-college-precollege-scan.json` if you can write files.
