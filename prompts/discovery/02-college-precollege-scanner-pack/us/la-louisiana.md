# Copy-Paste Discovery Prompt: US / LA / Louisiana — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-la-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-la-college-precollege-scan`
- `country`: `US`
- `region`: `LA`
...
- `site:.edu "Louisiana" "pre-college" residential`
- `site:.edu "Louisiana" "summer program" residential overnight`
- `site:.edu "Louisiana" "summer immersion" OR "summer institute" residential`
- `"Louisiana" university "summer camp" residential "high school"`
- `"Louisiana" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-la-college-precollege-scan.json` if you can write files.
