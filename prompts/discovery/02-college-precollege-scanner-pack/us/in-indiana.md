# Copy-Paste Discovery Prompt: US / IN / Indiana — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-in-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-in-college-precollege-scan`
- `country`: `US`
- `region`: `IN`
- `region_name`: `Indiana`
...
- `site:.edu "Indiana" "pre-college" residential`
- `site:.edu "Indiana" "summer program" residential overnight`
- `site:.edu "Indiana" "summer immersion" OR "summer institute" residential`
- `"Indiana" university "summer camp" residential "high school"`
- `"Indiana" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-in-college-precollege-scan.json` if you can write files.
