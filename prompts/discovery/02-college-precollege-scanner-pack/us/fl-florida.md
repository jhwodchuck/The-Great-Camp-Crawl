# Copy-Paste Discovery Prompt: US / FL / Florida — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-fl-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-fl-college-precollege-scan`
- `country`: `US`
- `region`: `FL`
- `region_name`: `Florida`
...
- `site:.edu "Florida" "pre-college" residential`
- `site:.edu "Florida" "summer program" residential overnight`
- `site:.edu "Florida" "summer immersion" OR "summer institute" residential`
- `"Florida" university "summer camp" residential "high school"`
- `"Florida" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-fl-college-precollege-scan.json` if you can write files.
