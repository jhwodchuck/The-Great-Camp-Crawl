# Copy-Paste Discovery Prompt: US / NV / Nevada — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-nv-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-nv-college-precollege-scan`
- `country`: `US`
- `region`: `NV`
- `region_name`: `Nevada`
...
- `site:.edu "Nevada" "pre-college" residential`
- `site:.edu "Nevada" "summer program" residential overnight`
- `site:.edu "Nevada" "summer immersion" OR "summer institute" residential`
- `"Nevada" university "summer camp" residential "high school"`
- `"Nevada" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-nv-college-precollege-scan.json` if you can write files.
