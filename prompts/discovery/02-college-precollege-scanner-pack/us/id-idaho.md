# Copy-Paste Discovery Prompt: US / ID / Idaho — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-id-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-id-college-precollege-scan`
- `country`: `US`
- `region`: `ID`
- `region_name`: `Idaho`
...
- `site:.edu "Idaho" "pre-college" residential`
- `site:.edu "Idaho" "summer program" residential overnight`
- `site:.edu "Idaho" "summer immersion" OR "summer institute" residential`
- `"Idaho" university "summer camp" residential "high school"`
- `"Idaho" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-id-college-precollege-scan.json` if you can write files.
