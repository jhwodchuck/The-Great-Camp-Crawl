# Copy-Paste Discovery Prompt: US / MO / Missouri — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-mo-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-mo-college-precollege-scan`
- `country`: `US`
- `region`: `MO`
...
- `site:.edu "Missouri" "pre-college" residential`
- `site:.edu "Missouri" "summer program" residential overnight`
- `site:.edu "Missouri" "summer immersion" OR "summer institute" residential`
- `"Missouri" university "summer camp" residential "high school"`
- `"Missouri" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-mo-college-precollege-scan.json` if you can write files.
