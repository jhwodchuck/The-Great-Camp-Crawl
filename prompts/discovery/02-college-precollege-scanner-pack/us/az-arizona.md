# Copy-Paste Discovery Prompt: US / AZ / Arizona — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/us-az-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `us-az-college-precollege-scan`
- `country`: `US`
- `region`: `AZ`
- `region_name`: `Arizona`
...
- `site:.edu "Arizona" "pre-college" residential`
- `site:.edu "Arizona" "summer program" residential overnight`
- `site:.edu "Arizona" "summer immersion" OR "summer institute" residential`
- `"Arizona" university "summer camp" residential "high school"`
- `"Arizona" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/us-az-college-precollege-scan.json` if you can write files.
