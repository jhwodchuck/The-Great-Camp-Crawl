# Copy-Paste Discovery Prompt: MX / QUE / Queretaro — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-que-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-que-college-precollege-scan`
- `country`: `MX`
- `region`: `QUE`
...
- `site:.mx "Queretaro" "programa preuniversitario" residencial`
- `site:.mx "Queretaro" "programa de verano" residencial preparatoria`
- `"Queretaro" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Queretaro" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Queretaro" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-que-college-precollege-scan.json` if you can write files.
