# Copy-Paste Discovery Prompt: MX / TLA / Tlaxcala — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-tla-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-tla-college-precollege-scan`
- `country`: `MX`
- `region`: `TLA`
...
- `site:.mx "Tlaxcala" "programa preuniversitario" residencial`
- `site:.mx "Tlaxcala" "programa de verano" residencial preparatoria`
- `"Tlaxcala" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Tlaxcala" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Tlaxcala" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-tla-college-precollege-scan.json` if you can write files.
