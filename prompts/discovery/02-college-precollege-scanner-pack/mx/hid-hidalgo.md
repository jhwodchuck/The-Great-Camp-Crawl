# Copy-Paste Discovery Prompt: MX / HID / Hidalgo — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-hid-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-hid-college-precollege-scan`
- `country`: `MX`
- `region`: `HID`
- `region_name`: `Hidalgo`
...
- `site:.mx "Hidalgo" "programa preuniversitario" residencial`
- `site:.mx "Hidalgo" "programa de verano" residencial preparatoria`
- `"Hidalgo" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Hidalgo" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Hidalgo" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-hid-college-precollege-scan.json` if you can write files.
