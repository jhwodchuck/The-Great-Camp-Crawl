# Copy-Paste Discovery Prompt: MX / NAY / Nayarit — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-nay-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-nay-college-precollege-scan`
- `country`: `MX`
- `region`: `NAY`
- `region_name`: `Nayarit`
...
- `site:.mx "Nayarit" "programa preuniversitario" residencial`
- `site:.mx "Nayarit" "programa de verano" residencial preparatoria`
- `"Nayarit" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Nayarit" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Nayarit" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-nay-college-precollege-scan.json` if you can write files.
