# Copy-Paste Discovery Prompt: MX / NLE / Nuevo Leon — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-nle-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-nle-college-precollege-scan`
- `country`: `MX`
- `region`: `NLE`
...
- `site:.mx "Nuevo Leon" "programa preuniversitario" residencial`
- `site:.mx "Nuevo Leon" "programa de verano" residencial preparatoria`
- `"Nuevo Leon" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Nuevo Leon" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Nuevo Leon" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-nle-college-precollege-scan.json` if you can write files.
