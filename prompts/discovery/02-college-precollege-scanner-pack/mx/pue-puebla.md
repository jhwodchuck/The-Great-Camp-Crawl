# Copy-Paste Discovery Prompt: MX / PUE / Puebla — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-pue-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-pue-college-precollege-scan`
- `country`: `MX`
- `region`: `PUE`
- `region_name`: `Puebla`
...
- `site:.mx "Puebla" "programa preuniversitario" residencial`
- `site:.mx "Puebla" "programa de verano" residencial preparatoria`
- `"Puebla" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Puebla" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Puebla" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-pue-college-precollege-scan.json` if you can write files.
