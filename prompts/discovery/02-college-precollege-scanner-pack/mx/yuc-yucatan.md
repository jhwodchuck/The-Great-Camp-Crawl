# Copy-Paste Discovery Prompt: MX / YUC / Yucatan — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-yuc-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-yuc-college-precollege-scan`
- `country`: `MX`
- `region`: `YUC`
- `region_name`: `Yucatan`
...
- `site:.mx "Yucatan" "programa preuniversitario" residencial`
- `site:.mx "Yucatan" "programa de verano" residencial preparatoria`
- `"Yucatan" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Yucatan" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Yucatan" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-yuc-college-precollege-scan.json` if you can write files.
