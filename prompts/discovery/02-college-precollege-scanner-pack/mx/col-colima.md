# Copy-Paste Discovery Prompt: MX / COL / Colima — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-col-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-col-college-precollege-scan`
- `country`: `MX`
- `region`: `COL`
- `region_name`: `Colima`
...
- `site:.mx "Colima" "programa preuniversitario" residencial`
- `site:.mx "Colima" "programa de verano" residencial preparatoria`
- `"Colima" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Colima" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Colima" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-col-college-precollege-scan.json` if you can write files.
