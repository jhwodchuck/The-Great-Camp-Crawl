# Copy-Paste Discovery Prompt: MX / MEX / Estado de Mexico — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-mex-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-mex-college-precollege-scan`
- `country`: `MX`
- `region`: `MEX`
- `region_name`: `Estado de Mexico`
- `language_mode`: `spanish-first; keep English evidence when encountered`
- `source_focus`: `official university program pages first (.mx .edu sites preferred); regional directories only for recall expansion`
- `max_candidates`: `25`

Search angles to cover:

- `site:.mx "Estado de Mexico" "programa preuniversitario" residencial`
- `site:.mx "Estado de Mexico" "programa de verano" residencial preparatoria`
- `"Estado de Mexico" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Estado de Mexico" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Estado de Mexico" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-mex-college-precollege-scan.json` if you can write files.
