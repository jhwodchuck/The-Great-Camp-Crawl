# Copy-Paste Discovery Prompt: MX / SON / Sonora — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-son-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-son-college-precollege-scan`
- `country`: `MX`
- `region`: `SON`
- `region_name`: `Sonora`
...
- `site:.mx "Sonora" "programa preuniversitario" residencial`
- `site:.mx "Sonora" "programa de verano" residencial preparatoria`
- `"Sonora" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Sonora" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Sonora" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-son-college-precollege-scan.json` if you can write files.
