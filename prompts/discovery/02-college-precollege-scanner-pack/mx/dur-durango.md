# Copy-Paste Discovery Prompt: MX / DUR / Durango — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-dur-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-dur-college-precollege-scan`
- `country`: `MX`
- `region`: `DUR`
- `region_name`: `Durango`
...
- `site:.mx "Durango" "programa preuniversitario" residencial`
- `site:.mx "Durango" "programa de verano" residencial preparatoria`
- `"Durango" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Durango" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Durango" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-dur-college-precollege-scan.json` if you can write files.
