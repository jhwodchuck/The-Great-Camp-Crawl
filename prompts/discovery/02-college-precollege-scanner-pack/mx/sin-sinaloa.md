# Copy-Paste Discovery Prompt: MX / SIN / Sinaloa — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-sin-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-sin-college-precollege-scan`
- `country`: `MX`
- `region`: `SIN`
- `region_name`: `Sinaloa`
...
- `site:.mx "Sinaloa" "programa preuniversitario" residencial`
- `site:.mx "Sinaloa" "programa de verano" residencial preparatoria`
- `"Sinaloa" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Sinaloa" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Sinaloa" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-sin-college-precollege-scan.json` if you can write files.
