# Copy-Paste Discovery Prompt: MX / JAL / Jalisco — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/mx-jal-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `mx-jal-college-precollege-scan`
- `country`: `MX`
- `region`: `JAL`
- `region_name`: `Jalisco`
...
- `site:.mx "Jalisco" "programa preuniversitario" residencial`
- `site:.mx "Jalisco" "programa de verano" residencial preparatoria`
- `"Jalisco" universidad "campamento" residencial "preparatoria" OR "bachillerato"`
- `"Jalisco" universidad "verano" "internado" OR "residencia" preparatoria`
- `"Jalisco" colegio "programa residencial" preparatoria`
...
- Save the JSON to `reports/discovery/mx-jal-college-precollege-scan.json` if you can write files.
