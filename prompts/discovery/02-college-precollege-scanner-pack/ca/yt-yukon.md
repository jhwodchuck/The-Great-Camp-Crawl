# Copy-Paste Discovery Prompt: CA / YT / Yukon — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-yt-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-yt-college-precollege-scan`
- `country`: `CA`
- `region`: `YT`
- `region_name`: `Yukon`
...
- `site:.ca "Yukon" "pre-college" residential`
- `site:.ca "Yukon" "summer program" residential overnight`
- `site:.ca "Yukon" "summer institute" OR "summer immersion" residential`
- `"Yukon" university "summer camp" residential "high school"`
- `"Yukon" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/ca-yt-college-precollege-scan.json` if you can write files.
