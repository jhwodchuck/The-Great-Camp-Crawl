# Copy-Paste Discovery Prompt: CA / NU / Nunavut — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-nu-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-nu-college-precollege-scan`
- `country`: `CA`
- `region`: `NU`
- `region_name`: `Nunavut`
...
- `site:.ca "Nunavut" "pre-college" residential`
- `site:.ca "Nunavut" "summer program" residential overnight`
- `site:.ca "Nunavut" "summer institute" OR "summer immersion" residential`
- `"Nunavut" university "summer camp" residential "high school"`
- `"Nunavut" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/ca-nu-college-precollege-scan.json` if you can write files.
