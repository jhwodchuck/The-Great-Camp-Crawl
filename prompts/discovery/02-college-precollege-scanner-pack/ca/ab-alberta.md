# Copy-Paste Discovery Prompt: CA / AB / Alberta — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-ab-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-ab-college-precollege-scan`
- `country`: `CA`
- `region`: `AB`
- `region_name`: `Alberta`
...
- `site:.ca "Alberta" "pre-college" residential`
- `site:.ca "Alberta" "summer program" residential overnight`
- `site:.ca "Alberta" "summer institute" OR "summer immersion" residential`
- `"Alberta" university "summer camp" residential "high school"`
- `"Alberta" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/ca-ab-college-precollege-scan.json` if you can write files.
