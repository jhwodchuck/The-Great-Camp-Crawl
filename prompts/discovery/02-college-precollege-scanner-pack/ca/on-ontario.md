# Copy-Paste Discovery Prompt: CA / ON / Ontario — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-on-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-on-college-precollege-scan`
- `country`: `CA`
- `region`: `ON`
- `region_name`: `Ontario`
...
- `site:.ca "Ontario" "pre-college" residential`
- `site:.ca "Ontario" "summer program" residential overnight`
- `site:.ca "Ontario" "summer institute" OR "summer immersion" residential`
- `"Ontario" university "summer camp" residential "high school"`
- `"Ontario" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/ca-on-college-precollege-scan.json` if you can write files.
