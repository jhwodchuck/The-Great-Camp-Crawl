# Copy-Paste Discovery Prompt: CA / QC / Quebec — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-qc-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-qc-college-precollege-scan`
- `country`: `CA`
- `region`: `QC`
- `region_name`: `Quebec`
- `language_mode`: `french-first; keep English evidence when encountered`
...
- `site:.ca "Quebec" "programme preuniversitaire" residentiel`
- `site:.ca "Quebec" "programme d'ete" residentiel secondaire`
- `"Quebec" universite "camp ete" residentiel secondaire`
- `"Quebec" college "programme residentiel" "secondaire" OR "lycee"`
- `"Quebec" university "summer program" residential "high school"`
...
- Save the JSON to `reports/discovery/ca-qc-college-precollege-scan.json` if you can write files.
