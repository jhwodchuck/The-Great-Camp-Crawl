# Copy-Paste Discovery Prompt: CA / MB / Manitoba — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-mb-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-mb-college-precollege-scan`
- `country`: `CA`
- `region`: `MB`
...
- `site:.ca "Manitoba" "pre-college" residential`
- `site:.ca "Manitoba" "summer program" residential overnight`
- `site:.ca "Manitoba" "summer institute" OR "summer immersion" residential`
- `"Manitoba" university "summer camp" residential "high school"`
- `"Manitoba" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/ca-mb-college-precollege-scan.json` if you can write files.
