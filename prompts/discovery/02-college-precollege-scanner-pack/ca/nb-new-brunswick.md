# Copy-Paste Discovery Prompt: CA / NB / New Brunswick — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-nb-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-nb-college-precollege-scan`
- `country`: `CA`
- `region`: `NB`
- `region_name`: `New Brunswick`
- `language_mode`: `bilingual English/French; preserve whichever language the source uses`
- `source_focus`: `official university and college program pages first (.ca .edu preferred); provincial associations only for recall expansion`
- `max_candidates`: `25`

Search angles to cover:

- `site:.ca "New Brunswick" "pre-college" residential`
- `site:.ca "New Brunswick" "summer program" residential overnight`
- `site:.ca "New Brunswick" "programme d'ete" residentiel secondaire`
- `"New Brunswick" university college "summer camp" residential "high school"`
- `"New Brunswick" universite "programme residentiel" secondaire`
...
- Save the JSON to `reports/discovery/ca-nb-college-precollege-scan.json` if you can write files.
