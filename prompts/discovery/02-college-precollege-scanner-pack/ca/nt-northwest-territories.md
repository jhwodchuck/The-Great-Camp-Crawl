# Copy-Paste Discovery Prompt: CA / NT / Northwest Territories — College Pre-College Scanner
...
- Save the gathered report to `reports/discovery/ca-nt-college-precollege-scan.json` if you have file-write access.
- If you do not have file-write access, return only the JSON object so it can be saved to that path.

Assigned slice:

- `run_slug`: `ca-nt-college-precollege-scan`
- `country`: `CA`
- `region`: `NT`
...
- `site:.ca "Northwest Territories" "summer program" residential overnight`
- `site:.ca "Northwest Territories" "summer institute" OR "summer immersion" residential`
- `"Northwest Territories" university "summer camp" residential "high school"`
- `"Northwest Territories" college "summer program" residential "boarding" OR "dorm"`
...
- Save the JSON to `reports/discovery/ca-nt-college-precollege-scan.json` if you can write files.
