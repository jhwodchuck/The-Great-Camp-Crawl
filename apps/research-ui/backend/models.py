...
class Camp(Base):
    ...
    is_excluded: Mapped[bool] = mapped_column(Boolean, nullable=True, default=False)
    exclusion_reason: Mapped[str] = mapped_column(String(128), nullable=True)
    exclusion_notes: Mapped[str] = mapped_column(Text, nullable=True)
    excluded_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
    excluded_by_user_id: Mapped[int] = mapped_column(Integer, nullable=True)
    triage_verdict: Mapped[str] = mapped_column(String(32), nullable=True)
    triage_confidence: Mapped[str] = mapped_column(String(16), nullable=True)
    triage_reason: Mapped[str] = mapped_column(Text, nullable=True)
    triaged_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
    triage_model: Mapped[str] = mapped_column(String(128), nullable=True)
    enriched_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
    enrichment_model: Mapped[str] = mapped_column(String(128), nullable=True)
    enrichment_source_file: Mapped[str] = mapped_column(String(512), nullable=True)
